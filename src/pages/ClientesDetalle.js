...
const ClientesDetalle = () => {
  ...
  const [sortBy, setSortBy] = useState('total'); // total, trabajos, antiguedad
    ...
    .sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'trabajos':
          ...
          break;
        case 'antiguedad':
          aValue = new Date(a.fechaPrimerTrabajo || 0);
          bValue = new Date(b.fechaPrimerTrabajo || 0);
          break;
        default:
          aValue = a.total || 0;
          bValue = b.total || 0;
  ...
  const totalIngresos = filteredAndSortedClientes.reduce((sum, c) => sum + (c.total || 0), 0);
  ...
  const bgPage = modoOscuro ? 'bg-[#0a0a0a]' : 'bg-[#f5f5f7]';
  ...
  return (
    <div className={`min-h-screen ${bgPage} ${textPrimary} transition-colors duration-300`}>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
        ...
        <div className="mb-4 sm:mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold">Mejores Clientes</h1>
              <p className={`${textSecondary} text-xs sm:text-sm`}>Analiza el rendimiento y distribución</p>
            </div>
          </div>
        </div>
          ...
          <div className={`${bgCard} rounded-xl p-3 sm:p-4 shadow-sm ${borderColor} border`}>
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                ...
                <p className="text-sm sm:text-lg font-bold text-emerald-500">{formatearMoneda(totalIngresos)}</p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2">
              {filteredAndSortedClientes.length === 0 ? (
                <div className={`${bgCard} rounded-xl p-6 shadow-sm ${borderColor} border text-center`}>
                  <UserIcon className={`w-12 h-12 mx-auto mb-2 ${textMuted}`} />
                ...
                filteredAndSortedClientes.map((cliente, index) => {
                  const porcentaje = totalIngresos > 0 ? (cliente.total / totalIngresos) * 100 : 0;
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`px-1.5 py-0.5 text-[10px] font-semibold rounded ${modoOscuro ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-800'}`}>
                              #{index + 1}
                            </span>
                          </div>
                          <div className={`flex items-center gap-3 text-xs ${textMuted}`}>
                            <span className="flex items-center gap-1">
                              ...
                              {cliente.cantidadTrabajos || 0} trabajos
                            </span>
                            <span className="flex items-center gap-1">
                              <CalendarIcon className="w-3 h-3" />
                              {formatearFecha(cliente.fechaPrimerTrabajo)}
                            </span>
                            <span className="flex items-center gap-1">
